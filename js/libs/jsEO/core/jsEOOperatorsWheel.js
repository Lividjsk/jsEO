...
var jsEOOperatorsWheel = new Class({
	...
	addOperator: function(_operator) {
		...
		jsEOUtils.debugln("Adding to jsEOOperatorsWheel a new operator with appRate  " + _operator.getApplicationRate())
		return this;
	},
	...
	setOperatorAt: function(_i, _operator) {
		if (typeof _i === 'number' && _i >= 0 && _i < this.operators.length) {
			this.appRateSum -= parseFloat(this.operators[i].getApplicationRate()) + parseFloat(_operator.getOperatorRate());
	...
	operate: function() {
		...
		jsEOUtils.debugln("Selecting operator " + i + " " + this.appRateSum + " " + rnd);
		return this.operators[i];
	}
});