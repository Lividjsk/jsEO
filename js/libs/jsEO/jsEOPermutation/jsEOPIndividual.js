...
var jsEOPIndividual = new Class({
	...
	solution4Q: function() {
		var chr = [new jsEOPosition(2, 1), new jsEOPosition(4, 2), new jsEOPosition(1, 3), new jsEOPosition(3, 4)];
	...
	show: function() {
		...
		for (var j = 0; j < chr.length; ++j) {
			if (j != chr.length - 1) cadena += chr[j].getPosition() + ",";
			else cadena += chr[j].getPosition();
		}
		return cadena;
	}
});