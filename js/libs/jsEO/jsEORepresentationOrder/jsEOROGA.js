...
var jsEOROGA = new Class({
	...
	run: function(_fitFn) {
		...
		this.indivSelector = new jsEOOpSelectorTournament(this.tournamentSize, Math.floor(this.popSize * this.replaceRate));

		this.operSelector = new jsEOOperatorsWheel();
		this.operSelector.
		addOperator(new jsEOROOpCrossOver(this.xOverRate));
		this.operSelector.
		addOperator(new jsEOROOpMutation(this.mutRate, this.mutPower, this.minValue, this.maxValue));
		if (this.opGet) {
